...
def build_lot_size_lookup(lot_data):
    ...
    for row in lot_data:
        idx = row['instrument']
        if not idx in lookup: lookup[idx] = []
...
def run_pnl_refresh():
    ...
    msg_start = f"🔄 Starting Highly Optimized Production P&L Engine (Chunk {chunk_index+1}/{total_chunks})..."
    print(msg_start)
    ...
    master_data_sorted = sorted(master_res.data, key=lambda x: int(x['strategy_id']))
    if total_chunks > 1:
        chunk_size = math.ceil(len(master_data_sorted) / total_chunks)
        start_idx = chunk_index * chunk_size
        end_idx = start_idx + chunk_size
        assigned_master_data = master_data_sorted[start_idx:end_idx]
    else:
        assigned_master_data = master_data_sorted

    if not assigned_master_data:
    ...
    print(f"📦 Verifying integrity and state for {len(assigned_master_data)} strategies in this chunk...")

    # 2. THE ONE-TOUCH LOOP
    for s in assigned_master_data: # CHANGED: Loop over chunk slice instead of all
        ...
        if res.data:
            ...
            if saved_cap != master_cap:
                if True: # TEMPORARY OVERRIDE: Forced True so ALL strategies undergo recalculation
                    print(f"   ⚠️ CAPITAL MISMATCH for Active ID {sid} (Saved: {saved_cap} | Master: {master_cap}). Forcing soft recalculation.")
                    latest_date = '2000-01-01'
                    cum_pnl = 0.0
                    peak_pnl = 0.0
                    resets_triggered += 1
                # --- TEMPORARY BACKFILL: Skipped inactive sync block to force full rebuild ---
                # else:
                ...
                # --- TEMPORARY BACKFILL: Bypass memory to force full history recalculation ---
        ...
        if True: # TEMPORARY OVERRIDE: Forced True to load math arrays for ALL strategies
    ...
    # 6. Final Summary
    print(f"\n--- Chunk {chunk_index} Processing Summary ---")
    ...
    if total_upserted > 0:
        msg_final = f"✅ Chunk {chunk_index} successfully upserted {total_upserted} daily trades."
        print(msg_final)
        report_progress("success", msg_final)
    else:
        msg_done = f"✅ Chunk {chunk_index} already up to date. No new active records were inserted."
        print(msg_done)
