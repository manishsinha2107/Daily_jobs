...
def calculate_intraday_pnl_1min_closing():
    ...
    print(f"🔍 Fetching database state for 1-Min Closing Audit... (Chunk {chunk_index+1}/{total_chunks})")
    report_progress("running", f"🔍 Analyzing P&L queue (Chunk {chunk_index+1}/{total_chunks})...")
    ...
    # Tracking total progress for the dashboard (Filtered for this chunk)
    total_dates = sum(len(set(strategy_map[s_id])) for s_id in assigned_strats)
    processed_count = 0

    # Iterating only over the strategies assigned to this chunk
    for strat_id in assigned_strats:
        ...
        for t_date in date_list:
            ...
            try:
                # --- PRE-CALCULATION CHECK (Temporarily Commented Out) ---
    ...
    # --- REPORTING SUCCESS ---
    report_progress("success", f"✅ Chunk {chunk_index} processed {processed_count} P&L dates.")
